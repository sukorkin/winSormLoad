...
where lower(client_type_name) like 'массовый рынок%'
and imsi not like '25096%'
--where lower(s.CLIENT_TYPE_NAME) not like '%тест%' and lower(s.CLIENT_TYPE_NAME) not like '%служебный%'
),
call_charge as (
select dim_subs_id, month_id, DIM_ITDDESC_ID, Wovat_$, stype_id, serv_code, serv_name, vol_in, vol_out, vol_in+vol_out vol_in_out
from wm_dwh.FCT_CALL_CHARGE_DAY_AGG ch
    inner join wm_dwh.dim_times dt on ch.time_id = dt.time_id
    ...
    inner join wm_dwh.dim_client dc on dc.dim_clnt_id = ch.DIM_CLNT_ID
        --and lower(dc.CLIENT_TYPE_NAME) not like '%тест%' and lower(dc.CLIENT_TYPE_NAME) not like '%служебный%'
        and lower(dc.client_type_name) like 'массовый рынок%'
    left join wm_dwh.dim_service ds on ch.dim_serv_id = ds.dim_serv_id
),
a as (
select/*parallel(12)*/ s.ext_id, s.activation_date, s.trpl_id as trpl,
    ...
    count(case when month_id = to_date('01.01.2020') and stype_id = 1 and lower(serv_name) like '%телефония%' then 1
          ...
          and serv_code = 22 then 1 else null end) calls_this_month,
    ...
    count(case when month_id = add_months(to_date('01.01.2020'), -1) and stype_id = 1 and lower(serv_name) like '%телефония%' then 1
          ...
          and serv_code = 22 then 1 else null end) calls_p_month,
    ...
    count(case when month_id = add_months(to_date('01.01.2020'), -2) and
        stype_id = 1 and lower(serv_name) like '%телефония%' then 1
          ...
          and serv_code = 22 then 1 else null end) calls_pp_month,
    ...
    count(case when month_id = add_months(to_date('01.01.2020'), -3) and
        stype_id = 1 and lower(serv_name) like '%телефония%' then 1
          ...
          and serv_code = 22 then 1 else null end) churn_calls_pp_month,
...
from b
    left join wm_dwh.dim_subscriber ds on b.subs_id = ds.ext_id
    join wm_dwh.dim_tariff_plan tp on tp.ext_id = b.trpl
),
d as (
...
sum(c.wovat_$) wovat_$
from wm_dwh.FCT_CALL_CHARGE c
join wm_dwh.dim_service s on c.dim_serv_id = s.dim_serv_id
join wm_dwh.dim_subscriber sc on c.dim_subs_id = sc.dim_subs_id
join wm_dwh.dim_tariff_plan tp on c.dim_trpl_id = tp.dim_trpl_id and tp.trpl_desc = 'B2C'--on c.dim_subs_id = sc.dim_subs_id
join wm_dwh.dim_clch_type ct on c.dim_clch_id = ct.dim_clch_id --and ct.ext_code = 'CALL_TYPE'  --on c.dim_clch_id = ct.dim_clch_id
where lower(sc.client_type_name) like 'массовый рынок%'
--and s.stype_id = 1
...
from traffic
where s_ext_id in (2194, 1288, 1916)
...
from traffic
where s_ext_id in (2, 2194, 1288, 1916)
